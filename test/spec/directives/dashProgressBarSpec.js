...
describe('Directive: dashProgressBar', function () {
  ...
  it('should create the progress bar div', inject(function($compile, $rootScope, $templateCache) {
    ...
    expect(element.html()).toEqual(
        '<div class="progress"><div class="bar" ng-style="{width: percentDone()}"></div></div>');
    $rootScope.$digest();
    expect(element.html()).toEqual(
        '<div class="progress"><div class="bar" ng-style="{width: percentDone()}" style="width: 20%;"></div></div>');
  }));
  ...
  it('should update the value based on progress change', inject(function($compile, $rootScope) {
    ...
    $rootScope.$digest();
    expect(element.html()).toEqual(
        '<div class="progress"><div class="bar" ng-style="{width: percentDone()}" style="width: 50%;"></div></div>');
  }));
  ...
  it('should handle invalid directive usage', inject(function($compile, $rootScope, $exceptionHandler) {
    ...
    $rootScope.$digest();
    expect(element.html()).toEqual(
        '<div class="progress"><div class="bar" ng-style="{width: percentDone()}" style="width: 0%;"></div></div>');
